  ...
  return (
    <nav style={{
      ...
      <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none' }}>
        ...
        <span style={{ fontFamily: 'Georgia, "Times New Roman", serif', fontWeight: 700, fontSize: 26, letterSpacing: '-0.02em', color: '#F7F6F3' }}>
          Steward
        </span>
      </Link>

      {/* Right side links */}
      <div className="blog-nav-links" style={{ display: 'flex', gap: 34, alignItems: 'center' }}>
        {PLAIN_LINKS.map(([href, label]) => (
          <Link key={href} href={href}
            style={{ fontSize: 17, fontWeight: 600, color: 'rgba(247,246,243,0.7)', textDecoration: 'none', transition: 'color .25s' }}
        ...
        <Link href={APP_STORE_URL} target="_blank" rel="noopener noreferrer"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 15.5, fontWeight: 600, color: '#F7F6F3', textDecoration: 'none', padding: '11px 22px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.2)', transition: 'all .25s' }}>
        ...
        {/* Get Started */}
        <Link href="/signup"
          style={{ background: '#6EE7B7', color: '#0F2018', fontWeight: 700, padding: '12px 28px', borderRadius: 12, fontSize: 16, textDecoration: 'none', display: 'inline-block' }}>
          Get Started for Free
        </Link>
      </div>

      <style>{`
        /* Bump the breakpoint a bit: bigger link sizes + gaps mean the nav
           needs more horizontal room before it starts to wrap/collide with
           the logo. Hide the right-side group below 1200px. */
