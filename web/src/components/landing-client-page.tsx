    ...
    <nav style={{
      ...
      <div className="lnd-nav-links" style={{ display: 'flex', gap: 28, alignItems: 'center' }}>
        ...
        >
          <button
            ...
            style={{ fontSize: 15, fontWeight: 500, color: resourcesOpen ? S.mint : 'rgba(247,246,243,0.55)', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontFamily: 'inherit', transition: 'color .25s', padding: 0 }}
          >
            Resources
      ...
      {menuOpen && (
        <div id="lnd-mobile-menu" style={{ width: '100%', borderTop: '1px solid rgba(110,231,183,0.08)', padding: '12px 0 16px', display: 'flex', flexDirection: 'column' as const, gap: 2 }}>
          ...
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: S.mint, padding: '14px 4px 6px', opacity: 0.6 }}>Resources</div>
          {RESOURCE_ITEMS.map(item => (
            <Link key={item.label} href={item.href} onClick={() => setMenuOpen(false)}
              style={{ fontSize: 15, fontWeight: 500, color: 'rgba(247,246,243,0.7)', textDecoration: 'none', padding: '11px 4px', borderBottom: '1px solid rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div className="lnd-hero-text" style={{ position: 'relative', zIndex: 10, maxWidth: 560, paddingLeft: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' as const, opacity: 0, transform: 'translateY(25px)', animation: 'hiw-fadeUp .8s 1.6s ease forwards' }}>
            ...
            <a href={APP_STORE_URL} target="_blank" rel="noopener noreferrer" className="lnd-appstore-hero"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(110,231,183,0.25)', color: S.cream, fontSize: 16, fontWeight: 700, padding: '18px 36px', borderRadius: 14, textDecoration: 'none', transition: 'all .35s cubic-bezier(.34,1.56,.64,1)' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill={S.mint}><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>
              iOS App
            </a>
          </div>
