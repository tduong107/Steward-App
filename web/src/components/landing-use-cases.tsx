...
const CARDS: Card[] = [
  {
    ...
    detail: { emoji: '📉', title: 'Price Drops', desc: 'Set a target price across thousands of retailers. Steward monitors 24/7 and pings you the second it hits your number — with fake deal detection so you never get played by inflated "sale" prices.', sim: '<strong>Price dropped!</strong> Nike Dunk Low Panda is now $89 at nike.com. That\'s 26% below your target.' },
  },
  {
    ...
    tag: '✓ Resy & more',
    preview: '🎉 Table found!',
    detail: { emoji: '🍽', title: 'Restaurant Tables', desc: 'That impossible Resy reservation? Steward watches it around the clock and pings you the moment a table frees up — so you can book before anyone else even knows it\'s available.', sim: '<strong>Table found!</strong> Carbone NY just opened a Friday 8pm slot for 2 guests.' },
  },
  {
    ...
    detail: { emoji: '✈️', title: 'Flight Deals', desc: 'Set a fare threshold for any route and Steward monitors prices across airlines. The moment it drops below your target, you get an instant alert with a direct link to book.', sim: '<strong>Fare dropped!</strong> SFO → Tokyo round trip is now $1,120. That\'s $127 less than yesterday.' },
  },
  {
    ...
    detail: { emoji: '🏕', title: 'Campsites', desc: 'The best campsites book up in seconds. Steward watches Recreation.gov for cancellations on specific dates and loops — and pings you the moment one opens up so you can grab it first.', sim: '<strong>Site available!</strong> Yosemite Upper Pines has an opening Jun 14–16. Book it before it\'s gone.' },
  },
  {
  ...
  },
  {
    ...
    detail: { emoji: '📦', title: 'Restocks', desc: 'Limited drops, sold-out sneakers, viral products that vanish in minutes. Steward monitors stock status and alerts you the moment something is back — so you\'re always first in line.', sim: '<strong>Back in stock!</strong> PS5 Pro is available at Target right now. Grab it before it sells out again.' },
  },
  {
    ...
    <section id="why-steward" style={{ padding: '120px 60px', background: '#080A08' }}>
      ...
      {modal && (
        ...
        >
          ...
          >
            <button onClick={() => setModal(null)} style={{ position: 'absolute', top: 16, right: 16, width: 32, height: 32, borderRadius: '50%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: 'rgba(247,246,243,0.5)', cursor: 'pointer', transition: 'all .25s', fontFamily: 'inherit' }}>✕</button>
            ...
            <div style={{ background: 'rgba(110,231,183,0.06)', border: '1px solid rgba(110,231,183,0.15)', borderRadius: 16, padding: 16, marginBottom: 20, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
              ...
              <div style={{ fontSize: 13, color: 'rgba(247,246,243,0.7)', lineHeight: 1.45 }} dangerouslySetInnerHTML={{ __html: modal.sim }} />
            </div>
